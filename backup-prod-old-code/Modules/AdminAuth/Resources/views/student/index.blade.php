...
@section('after_scripts')
    ...
    <script type="text/javascript">
        ...
        function actionColumn(value, row, index) {
            ...
            <?php
                $hp = \App\Helpers\Auth::has_permission('student.show-reset-password', $user, $permissions);
                if ($hp) { ?>
            ...
            <?php
            $hp = \App\Helpers\Auth::has_permission('student.index', $user, $permissions);
            if ($hp) { ?>
            ...
            editBtn.push('<a href="{{ route('adminauth::profile.index') }}?id=' + value + '" class="add-tooltip btn btn-xs btn-warning" data-id="' + value + '" data-toggle="tooltip" title="Xem hồ sơ">' +
                '<i class="fa fa-address-card-o" aria-hidden="true"></i></a>');

            editBtn.push('<a style="background-color:#00b3ca;border-color:#00bcd4" href="{{ route('adminauth::profile.study_broad_profile') }}?id=' + value + '" class="add-tooltip btn btn-xs btn-warning" data-id="' + value + '" data-toggle="tooltip" title="Xem thông tin du học">' +
