  ...
  return (
    ...
    >
      <div className={`absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] ${hasIcon ? 'flex w-full justify-center' : ''}`}>
        ...
        <span className={`uppercase font-[400] ${styleButton === EStyleButton.Normal ? 'text-[2rem]' : 'text-[3.2rem]'}`}>DOWNLOAD APP</span>
        {styleButton === EStyleButton.CommingSoon && <p className={"m-0 uppercase font-[400] text-[2rem] tracking-[10px]"}>COMING SOON</p>}
      </div>
    </div>
  )
}
