...
const InputRequest = ({ data }) => {
  const arrData =
    data &&
    Object.values(data).map((element) => {
      return {
        ...element,
        key: slugify(element?.title),
      };
    });
  const InputArrData =
    arrData &&
  ...
  //                   style={{ borderBottom: '1px solid #DEE2E6' }}
  //                 >
  //                   <span className={'text-body-16 text-style-12'}>
  //                     Học phí
  //                   </span>
  //                   <span className={'text-body-16 text-style-12'}>
  //                     £15,900
  ...
  //   },
  // ];
  return <Tabs options={InputArrData} defaultKey={InputArrData?.[0]?.key} />;
};
