...
class ProfileController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data['title']           = 'Hồ sơ';
        $this->data['controllerName']  = 'adminauth::profile';
    }
    ...
    public function search(Request $request)
    {
        ...
        $objects = ProfileSubmited::with('school')->select('profile_submited.*');
        $keyword = $request->input('search', '');
        if ($keyword) {
            $objects->where(function ($query) use ($keyword) {
                $query->where('profile_submited.name', 'LIKE', '%' . $keyword . '%');
                $query->orWhere('profile_submited.email', 'LIKE', '%' . $keyword . '%');
                $query->orWhere('profile_submited.phone', 'LIKE', '%' . $keyword . '%');
            });
        }
        $student_id = $request->input('student_id', '');
        if(!empty($student_id))
        {
            $objects->where("user_id", $student_id);
        }
        $sort = $request->input('sort', 'profile_submited.created_at');
    ...
    {
        $this->data['title'] = 'Xem hồ sơ';
        $profile = ProfileSubmited::find($id);
        $this->data['id'] = $id;
        $this->data['profile'] = $profile;
        return view("{$this->data['controllerName']}.view ",$this->data);
    }

    public function postView(Request $request){
       ProfileSubmited::where('id', $request->id)->update(['status' => $request->status]);
    ...
    //
    public function study_broad_profile(Request $request)
    {
       $id = $request->input('id',0);
       $infor = ProfileAbroad::where('user_id',($id))->first();
       return view('adminauth::study-abroad-profile.index',compact('infor'));
    }
